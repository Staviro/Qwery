...
const qwery_music = new Qwery({ name: "music", log: true }).create();

jsert_music.add("Adding all songs from 2024 in one playlist called '2024'", function() {
    const ds_playlist = "playlist 2024";
    let songsOf2024 = tracks.filter(x => x.releaseYear == 2024);
    qwery_music.addList({ dataset: ds_playlist, data: songsOf2024 });
    let data = qwery_music.getAll({ dataset: ds_playlist });
    if (data.length == songsOf2024.length) {
        jsert_music.pass(this);
    } else {
        jsert_music.fail(this);
    }
    qwery_music.reset();
